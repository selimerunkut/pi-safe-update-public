...
## Hidden Executable Code / Visual Camouflage
- Do not trust visual review alone for config/build files.
- Inspect executable config files directly: `tailwind.config.*`, `vite.config.*`, `next.config.*`, `webpack.config.*`, `postcss.config.*`, `eslint.config.*`, `babel.config.*`, `rollup.config.*`, `tsup.config.*`, `vitest.config.*`, `jest.config.*`, `playwright.config.*`, `cypress.config.*`, `package.json`.
- Run deterministic scans for:
  - long whitespace followed by code,
  - invisible/control Unicode,
  - risky JS execution primitives (`eval`, `new Function`, `child_process`, `execSync`, `spawnSync`, `fetch`, `XMLHttpRequest`),
  - suspicious base64 decode / obfuscation patterns.
- Use `./scripts/hidden-code-scan.sh` as the default deterministic scan helper.
- If a file still looks suspicious, use the project formatter or a formatted diff as a helper to reveal hidden payloads. Treat formatter output as secondary evidence, not the detector.
- Never say a repo is clean from model-only inspection; report the exact commands and files reviewed.
